function result = ISDIC_Serial_QR_func(Tx, Rx)
    % ISDIC_Serial_QR_func_Low_Complexity
    format shortE;
    warning('off','all');
    tic
            ...
            for iteration = 1:5
                for i=order
                    ...
                    for ii = order    
                        x = D(ii, ii, i) * R(:,ii);
                        y_H = R_H(ii,:);
                        A_next = inv_A - ((inv_A * x * y_H * inv_A)/(1 + (y_H * inv_A * x)));
                        inv_A = A_next;
                    end
                    %% check
%                     temp = inv(h * D(:,:,i) * conj(h.') + N * eye(Rx));
%                     disp(inv_A == temp)
...
%                     f(:,:,i) = conj(h(:,i).') * inv(h * D(:,:,i) * conj(h.') + N * eye(Rx));
    ...
    fileName = strcat(pwd,'\result\', 'ISDIC_', string(Tx), 'x', string(Rx), '_QR_LC', '.mat');
