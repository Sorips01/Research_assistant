function [location] = Receiver_SIC_MMSE(Tx, ~,rx_sym, ch, noise_va, symset, ~, ~, ordering)
...
for i=1:Tx
    ...
    filter = (ch_product+noise_va*eye(current_Tx))\ch_buf';
    if ordering == 1
        value_for_ordering = current_Tx:-1:1;
    elseif ordering == 2
        value_for_ordering = diag(real(ch_product)); %% More accurate MMSE SINR: diag(real(ch_product))
    elseif ordering == 3
        value_for_ordering = diag(real(filter*ch_buf)); %% More accurate MMSE SINR: diag(real(ch_product))
    end    
